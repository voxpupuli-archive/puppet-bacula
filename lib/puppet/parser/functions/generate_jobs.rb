module Puppet::Parser::Functions
  newfunction(:generate_jobs) do |args|
    ...
    self.to_hash.each do |variable,value|
      if variable =~ /^bacula_job_.*$/
        ...
        function_create_resources('bacula::config::jobs', {name => parameters})
      end
    end

    begin
      function_create_resources('bacula::config::jobs', args[0])
    rescue => e
